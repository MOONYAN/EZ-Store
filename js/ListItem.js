...
firebase.database().ref('item').on('child_added',function (snapshot) {
        ...
        var $item = $('<div class="col-xs-12 col-sm-4 list-group"></div>');
        console.log(snapshot.key);
        var $id = $(String.format('<div class="list-group-item">ItemId:{0}</div>', snapshot.key));
        console.log(snapshot.Name);
        var $name = $(String.format('<div class="list-group-item">Name:{0}</div>', data.Name));
        console.log(snapshot.UnitPrice);
        var $price = $(String.format('<div class="list-group-item">UnitPrice:{0}</div>', data.UnitPrice));
        console.log(snapshot.ImageURL);
        var $image = $(String.format('<div><img class="img-circle" alt="imageSource" src={0} /><div>', data.ImageURL));
